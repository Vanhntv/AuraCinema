...
function CounterSale() {
  ...
  const print = async () => {
    if (!sale || sale.printed || printing) return;
    setPrinting(true);
    setError("");
    try {
      const data = await api(`/staff/pos/sales/${sale.booking_id}/print`, { method: "POST" });
      const QRCode = (await import("qrcode")).default;
      const tickets = await Promise.all(data.tickets.map(async (ticket) => ({
        ...ticket,
        qr_image: await QRCode.toDataURL(ticket.qr_payload, { errorCorrectionLevel: "M", margin: 1, width: 240 }),
      })));
      setPrintDocument({ ...data, tickets });
      setSale((currentSale) => currentSale ? { ...currentSale, printed: true } : currentSale);
