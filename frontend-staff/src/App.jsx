...
function CounterSale() {
  ...
  useEffect(() => { let live = true; api("/staff/pos/showtimes").then((data) => live && setShowtimes(data)).catch((err) => live && setError(err.message)).finally(() => live && setLoading(false)); return () => { live = false; }; }, []);
  ...
  return <div className="pos-layout"><section className="pos-workspace"><Step number="1" title="Chọn phim và suất chiếu" text={loading ? "Đang tải suất chiếu..." : `${showtimes.length} suất chiếu khả dụng`} />{!loading && <div className="showtime-picker">{showtimes.length ? showtimes.map((showtime) => <button className={`showtime-card ${current?.id === showtime.id ? "selected" : ""}`} onClick={() => choose(showtime)} key={showtime.id}><span className="poster poster-one">{showtime.movie.title.slice(0, 2).toUpperCase()}</span><span><strong>{showtime.movie.title}</strong><small>{dateTime(showtime.start_time)}</small><small>{showtime.room.cinema} · {showtime.room.name}</small></span><em>{showtime.available_seats} ghế trống</em></button>) : <p className="empty-note">Không có suất chiếu khả dụng.</p>}</div>}{current && <><Step number="2" title="Chọn ghế" text="Chỉ ghế màu tím còn trống mới có thể bán." />{seatLoading ? <p className="empty-note">Đang tải sơ đồ ghế...</p> : <SeatMap seats={seats} selected={selected} toggle={toggle} />}</>}</section><aside className="order-panel"><h2>Thông tin đơn hàng</h2>{current ? <><div className="order-row"><span>Phim</span><strong>{current.movie.title}</strong></div><div className="order-row"><span>Suất chiếu</span><strong>{dateTime(current.start_time)}</strong></div><div className="order-row"><span>Phòng</span><strong>{current.room.name}</strong></div><div className="order-seats"><span>Ghế đã chọn</span><div>{chosen.length ? chosen.map((seat) => <b key={seat.id}>{seat.label}</b>) : "Chưa chọn ghế"}</div></div><div className="order-total"><span>Tổng thanh toán</span><strong>{money.format(total)}</strong></div><div className="payment-method">✓ Thanh toán tiền mặt</div><button className="pay-button" disabled={!selected.length || busy} onClick={pay}>{busy ? "Đang xử lý..." : `Xác nhận thanh toán ${money.format(total)}`}</button></> : <p className="empty-note">Hãy chọn một suất chiếu để bắt đầu.</p>}{sale && <div className="sale-success"><strong>Thanh toán thành công</strong><span>Mã đơn: {sale.booking_code}</span><span>Vé: {sale.tickets.map((ticket) => ticket.seat).join(", ")}</span><button onClick={print}>In vé cứng</button></div>}</aside>{error && <div className="pos-alert">{error}</div>}</div>;
}
...
function normalizeSeat(item) { const seat = item.seat_id || item.seat || {}; const label = String(seat.seat_code || `${seat.seat_row || ""}${seat.seat_number || ""}`).toUpperCase(); return { id: item._id || item.id, label, row: seat.seat_row || label.charAt(0) || "?", number: Number(seat.seat_number || label.slice(1) || 0), price: Number(item.price || 0), status: item.status }; }
function SeatMap({ seats, selected, toggle }) { const rows = [...new Set(seats.map((seat) => seat.row))].sort(); return <div className="seat-map"><div className="screen">MÀN HÌNH</div><div className="seat-legend"><span><i className="available" />Trống</span><span><i className="selected" />Đang chọn</span><span><i className="taken" />Đã bán / đang giữ</span><span><i className="maintenance" />Bảo trì</span></div>{rows.map((row) => <div className="seat-row" key={row}><b>{row}</b><div>{seats.filter((seat) => seat.row === row).sort((a, b) => a.number - b.number).map((seat) => <button key={seat.id} disabled={seat.status !== "available"} onClick={() => toggle(seat)} className={`seat ${seat.status} ${selected.includes(seat.id) ? "selected" : ""}`} title={`${seat.label} · ${seat.status === "maintenance" ? "Đang bảo trì" : money.format(seat.price)}`}>{seat.number || seat.label}</button>)}</div><b>{row}</b></div>)}</div>; }
