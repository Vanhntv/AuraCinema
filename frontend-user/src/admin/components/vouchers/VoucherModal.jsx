...
const VoucherModal = ({ isOpen, onClose, onSubmit, isLoading }) => {
      ...
      <div className="modal modal-large" onClick={(event) => event.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body voucher-form">
            <section className="voucher-form-section">
              ...
              <div className="form-group">
                ...
                <input className="form-input" type="url" placeholder="https://..." value={formData.image_url} onChange={(event) => handleChange("image_url", event.target.value)} />
              </div>
            </section>
            ...
            <section className="voucher-form-section">
              ...
              <div className="segmented-options">
                <label><input type="radio" checked={formData.discount_type === "percent"} onChange={() => handleChange("discount_type", "percent")} /> Giảm theo phần trăm</label>
                <label><input type="radio" checked={formData.discount_type === "fixed"} onChange={() => handleChange("discount_type", "fixed")} /> Giảm số tiền cố định</label>
              </div>
            </section>
            ...
            <section className="voucher-form-section">
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input className={`form-input ${errors.discount_value ? "error" : ""}`} type="number" min="1" value={formData.discount_value} onChange={(event) => handleChange("discount_value", event.target.value)} />
                ...
                <div className="form-group">
                  ...
                  <input className={`form-input ${errors.min_order ? "error" : ""}`} type="number" min="0" value={formData.min_order} onChange={(event) => handleChange("min_order", event.target.value)} />
            ...
            <section className="voucher-form-section">
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <input className={`form-input ${errors.usage_limit_per_user ? "error" : ""}`} type="number" min="1" value={formData.usage_limit_per_user} onChange={(event) => handleChange("usage_limit_per_user", event.target.value)} />
            ...
            <section className="voucher-form-section">
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <select className="form-input" value={formData.apply_scope} onChange={(event) => handleChange("apply_scope", event.target.value)}>
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" placeholder="Nhập ID phim, cách nhau bằng dấu phẩy" value={formData.applicable_movie_ids} onChange={(event) => handleChange("applicable_movie_ids", event.target.value)} />
                </div>
              </div>
              <div className="segmented-options">
                {memberTierOptions.map((tier) => (
                  <label key={tier}>
                    <input type="checkbox" checked={formData.applicable_member_tiers.includes(tier)} onChange={(event) => handleTierToggle(tier, event.target.checked)} />
                    {tier.toUpperCase()}
                  </label>
                ))}
              </div>
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn btn-primary" disabled={isLoading}>{isLoading ? "Đang lưu..." : "Tạo mã giảm giá"}</button>
          </div>
        </form>
      </div>
    </div>
