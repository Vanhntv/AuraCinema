  ...
  return (
    <main className="bg-[#0f141c] pb-24 pt-8 text-white">
      <div className="mx-auto w-[min(1120px,calc(100%_-_56px))] max-sm:w-[calc(100%_-_28px)]">
        ...
        {expired && (
          <div className="mb-6 rounded-[22px] border border-red-500/25 bg-red-500/10 px-4 py-4 text-sm leading-7 text-red-100">
            Khuyến mãi này đã hết hạn. Nội dung chỉ còn giá trị tham
            khảo.
          </div>
        )}
        ...
        <article className="overflow-hidden rounded-[30px] border border-white/10 bg-white/[0.03] shadow-[0_24px_80px_rgba(0,0,0,0.3)]">
          <div className="p-6 sm:p-8 lg:p-10">
            <div className="flex flex-wrap items-center gap-3 text-xs font-bold uppercase tracking-[0.18em] text-[#ff6070]">
              ...
              <span>{expired ? 'Đã hết hạn' : 'Đang áp dụng'}</span>
            </div>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <TimeChip label="Từ ngày" value={formatDateToDisplay(promotion.startDate)} />
              <TimeChip label="Đến ngày" value={formatDateToDisplay(promotion.endDate)} />
              <TimeChip label="Lượt xem" value={promotion.viewCount.toLocaleString('vi-VN')} />
            </div>
            ...
            <div className="mt-8 rounded-[24px] border border-white/10 bg-[#0f141c] p-5">
              ...
              <p className="mt-3 text-[15px] leading-8 text-slate-200">{promotion.summary}</p>
            </div>
